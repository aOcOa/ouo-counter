class Counter {
  constructor({
    ...
    this.generateElements().bindEvent();
  }
  reset(maxValue) {
  ...
  _handleCounterEvent(evt) {
    ...
    switch (action) {
      case "minus":
        if (currentValue - 1 < 1) {
          this.valueDom.textContent = 1;
          this.minusDom.className +=
            this.minusDom.className.indexOf(" is-disabled") !== -1
              ? ""
              : " is-disabled";
        } else {
          this.addDom.className = this.addDom.className.replace(
            " is-disabled",
            ""
          );
      ...
      case "add":
        if (currentValue + 1 <= this.maxValue) {
          this.minusDom.className = this.minusDom.className.replace(
            " is-disabled",
            ""
          );
          this.valueDom.textContent = currentValue + 1;
        } else {
          this.addDom.className +=
            this.addDom.className.indexOf(" is-disabled") !== -1
              ? ""
              : " is-disabled";
          this.handleMaxEvent();
        }
